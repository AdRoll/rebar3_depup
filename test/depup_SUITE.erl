...
-doc """
None of the deps should be updatable.
""".
no_updates(_) ->
...
-doc """
Deps should be updated, but the file should remain unchanged
""".
no_replace(_) ->
...
-doc """
This tests all the different kinds of possible updates in the default profile
""".
default_updates(_) ->
    ...
    [
        ...
        lists:usort(
            proplists:get_value(deps, UpdatedConfig) --
                proplists:get_value(deps, OriginalConfig)
        ),
    [{rebar3_gpb_plugin, _}] =
        lists:usort(
            proplists:get_value(plugins, UpdatedConfig) --
    ...
    [
        ...
        lists:usort(
            proplists:get_value(project_plugins, UpdatedConfig) --
...
-doc """
Only plugins should be updated
""".
just_plugins(_) ->
    {OriginalConfig, UpdatedConfig} =
...
-doc """
Only hex deps should be updated
""".
just_hex(_) ->
    {OriginalConfig, UpdatedConfig} =
...
-doc """
Don't update ignored deps on the command line
""".
ignore(_) ->
    {OriginalConfig, UpdatedConfig} =
...
-doc """
Don't update ignored deps in the rebar.config
""".
ignore_config(_) ->
    {OriginalConfig, UpdatedConfig} =
...
-doc """
Update if the patch version is the only change
""".
only_patch(_) ->
...
-doc """
Update if the minor (or patch) version is the only change
""".
only_minor(_) ->
    meck:new(dep_hex, [passthrough]),
    meck:expect(
...
-doc """
Update only if the version follows SemVer semantics
""".
only_major(_) ->
    meck:new(dep_hex, [passthrough]),
    meck:expect(
...
-doc """
Update if the only flag overrides the config options
""".
