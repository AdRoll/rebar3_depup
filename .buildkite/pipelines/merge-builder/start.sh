...
EOF
  ...
  results=$(curl \
    ...
    "https://api.github.com/repos/SemanticSugar/rebar3_depup/pulls?per_page=100&state=open" \
  ...
  for result in ${results}; do
    ...
    builds=$(curl \
      ...
      -H "Authorization: token ${GITHUB_TOKEN}" \
      ${status} \
      | jq '.[] | select(.target_url | contains("rebar3_depup-merge-builder")) | [.target_url] | @tsv' \
    )
    latest_build=0
...
cat <<EOF
    - curl -sX PUT "$(bk_url pipelines/rebar3_depup-merge-builder/builds/${latest_build}/rebuild)"
EOF
    fi
  done
else
  # Read the pipeline and output replacing variables
  sed -e "s/\$MERGE_BRANCH/main/g" ${root}/pipelines/pr-builder/start.yml
fi
